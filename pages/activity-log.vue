<template>
  <NuxtLayout name="dashboard">
    <div class="min-h-screen ">
      <div class="">
        <!-- Page Title -->
        <div class="mb-6 flex justify-between items-center rounded-xl bg-[#DCF1FF] text-[#005B8F] px-6 py-4">
          <div>
            <h1 class="text-xl font-bold bg-[#DCF1FF] text-[#005B8F]">Audit & Activity Log</h1>
            <p class="text-base bg-[#DCF1FF] text-[#005B8F] mt-1">Track system activities, chain of custody, and generate compliance reports.</p>
          </div>
          <div class="flex gap-2">
            <button @click="openExportModal"
              class="px-4 py-2 bg-[#005B8F] text-[#DCF1FF] font-semibold rounded-lg hover:bg-[#c0e7fb] transition text-sm shadow-sm flex items-center gap-2">``
        ...
        <div v-if="activeTab === 'audit'" class="space-y-6">
          <!-- Filters Section -->
          <div class="bg-white rounded-lg shadow-sm border-[0.5px] border-gray-100 p-4 md:p-6">
            <h3 class="text-sm font-semibold text-gray-900 mb-4">Filter Audit Logs</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <UiAnimatedInput v-model="auditFilters.userID" type="text" label="User ID" />
              </div>
              <div>
                <UiSelectInput label="Action Type" :options="['', 'CREATE', 'UPDATE', 'DELETE', 'LOGIN']" v-model="auditFilters.actionType" />
              </div>
              <div>
                <UiAnimatedInput v-model="auditFilters.keyword" type="text" label="Keyword" />
              </div>
              <div>
                <UiAnimatedInput v-model="auditFilters.startDate" type="date" label="Start Date" />
              </div>
              <div>
                <UiAnimatedInput v-model="auditFilters.endDate" type="date" label="End Date" />
              </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border-[0.5px] border-gray-100 overflow-hidden">
              ...
              <table class="min-w-full divide-y-[0.5px] divide-gray-100">
                <thead class="bg-gray-25">
                  <tr>
                    ...
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Log ID</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">User ID</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Action</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Entity</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Timestamp</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Actions</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y-[0.5px] divide-gray-100">
                  <tr v-for="(log, index) in paginatedAuditLogs" :key="log.logID" class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-900">{{ (auditCurrentPage - 1) * auditPageSize + index + 1 }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700 font-mono">{{ log.logID }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ log.userID }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm">
                      <span :class="getActionClass(log.actionType)" class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium">
                        {{ log.actionType }}
                      </span>
                    </td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ log.affectedEntity }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ formatDateTime(log.timestamp) }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm">
                      <button @click="viewDetails(log)" class="p-2 hover:bg-gray-100 rounded-lg transition" title="View Details">
            ...
            <!-- Pagination -->
            <Pagination 
              v-if="auditLogs?.data && auditLogs.data.length > 0"
        ...
        <div v-if="activeTab === 'custody'" class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-sm border-[0.5px] border-gray-100 p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <UiAnimatedInput v-model="custodyFilters.sampleID" type="text" label="Sample ID" />
              </div>
              <div>
                <UiAnimatedInput v-model="custodyFilters.userID" type="text" label="User ID" />
              </div>
              <div>
                <UiSelectInput label="Event Type" :options="['', 'RECEIVED', 'TRANSFERRED', 'ANALYZED', 'STORED', 'DISPOSED']" v-model="custodyFilters.eventType" />
              </div>
              <div class="flex items-end">
                <button @click="loadCustodyEvents"
                  class="w-full px-4 py-2.5 bg-[#005B8F] text-white rounded-lg hover:bg-[#004a73] transition text-sm font-medium">
                  Apply Filters
                </button>
              </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border-[0.5px] border-gray-100 overflow-hidden">
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y-[0.5px] divide-gray-100">
                <thead class="bg-gray-50">
                  <tr>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">SN</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Event ID</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Sample ID</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">User ID</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Event Type</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Notes</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Timestamp</th>
                    <th scope="col" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Actions</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y-[0.5px] divide-gray-100">
                  <tr v-for="(event, index) in paginatedCustodyEvents" :key="event.eventID" class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-900">{{ (custodyCurrentPage - 1) * custodyPageSize + index + 1 }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700 font-mono">{{ event.eventID }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ event.sampleID }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ event.userID }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm">
                      <span :class="getEventTypeClass(event.eventType)" class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium">
                        {{ event.eventType }}
                      </span>
                    </td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ event.notes || '-' }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm text-gray-700">{{ formatDateTime(event.timestamp) }}</td>
                    <td class="px-6 py-6 whitespace-nowrap text-sm">
                      <button @click="openReportModal(event.sampleID)" class="p-2 hover:bg-gray-100 rounded-lg transition" title="Generate Report">
                        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Pagination -->
            <Pagination 
              v-if="custodyEvents.length > 0"
              v-model:currentPage="custodyCurrentPage" 
              v-model:pageSize="custodyPageSize"
              :totalItems="custodyTotalItems" 
              :pageSizeOptions="[10, 25, 50, 100]" 
            />
          </div>
        </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showExportModal" class="fixed bg-black/50 backdrop-blur-lg inset-0 z-[60] overflow-y-auto" @click.self="closeExportModal">
          <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="closeExportModal"></div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showCustodyModal" class="fixed bg-black/50 backdrop-blur-lg inset-0 z-[60] overflow-y-auto" @click.self="closeCustodyModal">
          <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="closeCustodyModal"></div>
            ...
            <div class="relative w-full max-w-lg bg-white rounded-lg shadow-xl">
              ...
              <div class="px-6 py-6 space-y-4">
                <div>
                  <UiAnimatedInput v-model="custodyForm.sampleID" type="text" label="Sample ID" required />
                </div>
                <div>
                  <UiAnimatedInput v-model="custodyForm.userID" type="text" label="User ID" required />
                </div>
                <div>
                  <UiSelectInput label="Event Type" :options="['', 'RECEIVED', 'TRANSFERRED', 'ANALYZED', 'STORED', 'DISPOSED']" v-model="custodyForm.eventType" />
                </div>
                <div>
                  <UiAnimatedInput v-model="custodyForm.notes" label="Notes (Optional)" :rows="6" />
                </div>
              </div>

              <div class="flex justify-end gap-3 px-6 py-4 bg-gray-50 border-t border-gray-200">
                <button @click="closeCustodyModal"
                  class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                  Cancel
                </button>
                <button @click="handleRecordCustody" :disabled="custodySubmitting"
                  class="px-4 py-2 text-sm font-medium text-white bg-[#005B8F] rounded-md hover:bg-[#004a73] disabled:opacity-50">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showReportModal" class="fixed bg-black/50 backdrop-blur-lg inset-0 z-[60] overflow-y-auto" @click.self="closeReportModal">
          <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="closeReportModal"></div>
            ...
            <div class="relative w-full max-w-md bg-white rounded-lg shadow-xl">
              ...
              <div class="flex justify-end gap-3 px-6 py-4 bg-gray-50 border-t border-gray-200">
                ...
                <button @click="handleGenerateReport" :disabled="reportLoading"
                  class="px-4 py-2 text-sm font-medium text-white bg-[#005B8F] rounded-md hover:bg-[#004a73] disabled:opacity-50">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showDetailsModal" class="fixed bg-black/50 backdrop-blur-lg inset-0 z-[60] overflow-y-auto" @click.self="closeDetailsModal">
          <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="closeDetailsModal"></div>
...
const auditLogsData = computed(() => auditLogs.value?.data || [])
const custodyEventsData = computed(() => custodyEvents.value || [])
...
const {
  ...
  resetPagination: resetAuditPagination
} = usePagination(auditLogsData, {
  initialPage: 1,
  initialPageSize: 10,
  pageSizeOptions: [10, 25, 50, 100]
})

// Custody Events Pagination
const {
  currentPage: custodyCurrentPage,
  pageSize: custodyPageSize,
  totalItems: custodyTotalItems,
  paginatedItems: paginatedCustodyEvents,
  resetPagination: resetCustodyPagination
} = usePagination(custodyEventsData, {
  initialPage: 1,
  initialPageSize: 10,
  pageSizeOptions: [10, 25, 50, 100]
})
