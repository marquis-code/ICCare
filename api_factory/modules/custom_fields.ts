...
export const custom_fields_api = {
  ...
  $_create_custom_field: (payload: {
    ...
    return GATEWAY_ENDPOINT.post('/biospecimen/attributes/custom-fields/', payload)
  },

  // Get all custom attribute definitions
  $_get_custom_fields: (active_only: boolean = true) => {
    return GATEWAY_ENDPOINT.get('/biospecimen/attributes/custom-fields/', {
      params: { active_only }
    })
  },
  ...
  $_get_custom_field: (attribute_id: string) => {
    return GATEWAY_ENDPOINT.get(`/biospecimen/attributes/custom-fields/${attribute_id}`)
  }
}